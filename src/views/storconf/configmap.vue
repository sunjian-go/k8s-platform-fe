<template>
  <div>
          ...
          <el-card shadow="never" :body-style="{ padding: '10px' }">
            <div>
              ...
              >
                ...
                <el-table-column label="ConfigMap" align="left">
                  ...
                  <template v-slot="scope">
                    ...
                    <div>
                      <el-button
                        ...
                        plain
                        @click="
                          getConfigMapDetail(scope.row), (yamlDialog = true)
                        "
                        >YAML</el-button
                      >
                      <el-button
    ...
    <el-dialog title="创建Cm" v-model="scalestatus" width="25%" top="5%">
      <el-row>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="createCm">确 定</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    >
      <!-- 插槽：抽屉标题  -->
      <template #header>
        <span style="font-weight: bold; font-size: 18px">创建ConfigMap</span>
      </template>
      <!-- 插槽，抽屉body，填写表单属性 -->
      <template #default>
      ...
      <template #footer>
        <el-button
          ...
          >取消</el-button
        >
        <el-button type="primary" @click="submitForm('createConfigMap')"
          >立即创建</el-button
        >
      </template>
    </el-drawer>
...
export default {
  ...
  methods: {
    ...
    //获取configmap详情
    getConfigMapDetail(obj) {
      console.log("获取详情的为：", obj.metadata.name);
      this.configmapDetailData.params.name = obj.metadata.name;
      if (this.namespaceValue != "") {
        this.configmapDetailData.params.namespace = this.namespaceValue;
      }
      ...
      getConfigMapsDetailReq(this.configmapDetailData.params)
        .then((res) => {
          ...
          console.log("configmap详情YAML为：", this.jsontoyaml(res.data));
        })
        .catch((res) => {
    ...
    //提交configmap参数
    submitForm(formName) {
      //验证表单的每个规则是否通过，通过则调用createConfigMapFunc，反之返回false
    ...
    //获取cm列表
    getCms() {
      this.getCmData.params.name = this.searchValue;
      this.getCmData.params.namespace = this.namespaceValue;
      ...
      getConfigMapsReq(this.getCmData.params)
        .then((res) => {
          ...
          console.log("获取到：", res.data.items);
    ...
    deleteCm(cm) {
      ...
      this.deleteCmData.params.name = cm;
  ...
  watch: {
    ...
    namespaceValue: {
      handler() {
        ...
        // 页面刷新或切换的时候，把页数重新置为1并且重新获取configmap列表
        this.currentPage = 1;
        this.getCms();
      },
    },
