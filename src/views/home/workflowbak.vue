<template>
  ...
  <div>
    ...
    <!-- 抽屉1 -->
    <el-drawer v-model="drawer1" :before-close="handleClose1">
      <!-- 插槽：抽屉标题  -->
      <template #title>
        <span style="font-weight: bold; font-size: 18px">选择工作流类型</span>
      </template>
      <template #default>
        <div>
          <el-card shadow="never">
            <el-radio-group v-model="createWorkflowData.params.type">
          ...
          <el-col :span="20">
            ...
            >
              ...
              <el-form-item label="检查路径" class="workflow-create-form">
                <el-input v-model="createWorkflow.health_path"></el-input>
              </el-form-item>
              ...
              <el-form-item
                ...
                class="workflow-create-form"
              >
                <el-input v-model="createWorkflow.port"></el-input>
              </el-form-item>
              <el-form-item
                ...
                class="workflow-create-form"
              >
                <el-input v-model="createWorkflow.node_port"></el-input>
              </el-form-item>
              <div v-if="createWorkflowData.params.type == 'Ingress'">
                <el-form-item label="Ingress" class="workflow-create-form-item">
                </el-form-item>
                <el-form-item label="Ing域名" prop="host" style="width: 100%">
                  <el-input v-model="createWorkflow.host"></el-input>
                </el-form-item>
                <div
        ...
        <div>
          <el-card class="box-card">
            ...
            <div>
              <el-row :gutter="10">
                <el-col :span="8">
                  <div v-for="(data, i) in showInfo" :key="i">
                    <div v-if="data == '检查路径'">
                      <div v-if="createWorkflow.health_check == true">
                        {{ data }}
                      </div>
                    </div>
                    <div v-else-if="data == 'NodePort端口'">
                      <div v-if="createWorkflowData.params.type == 'NodePort'">
                        <span>{{ data }}</span>
                      </div>
                    </div>
                  ...
                  <div v-if="createWorkflowData.params.type == 'Ingress'">
                    ...
                    <div
                      v-for="(data, i) in createWorkflowData.params.hosts[
                        createWorkflow.host
                      ]"
                      :key="i"
                    >
                ...
                <el-col :span="10">
                  <div
                    v-for="(data, key) in createWorkflowData.params"
                    :key="key"
                  >
                    <div v-if="key == 'hosts'">
                      <div v-if="createWorkflowData.params.type == 'Ingress'">
                        <span>{{ createWorkflow.host }}</span>
                        <!-- 遍历hosts的规则数组 -->
                        <div
                          v-for="(chdata, i) in createWorkflowData.params.hosts[
                            createWorkflow.host
                          ]"
                          :key="i"
                        >
                          <!-- 遍历hosts里面的单个ing规则map -->
                          <div
                            v-for="(val, ckey) in createWorkflowData.params
                              .hosts[createWorkflow.host][i]"
                            :key="ckey"
                          >
                            <span>{{ val }}</span>
                          </div>
                      ...
                      <!-- <div v-else>
                          {{ (createWorkflowData.params.hosts = "") }}
                        </div> -->
                    </div>
                    <div v-else-if="key == 'node_port'">
                      <span
                        v-if="createWorkflowData.params.type == 'NodePort'"
                        >{{ data }}</span
                      >
...
export default {
  data() {
    return {
      ...
      //   showIngInfo: ["Ing域名", "主机路径", "路径类型", "SVC名字", "SVC端口"],
      testarr: [1],
      createIngress: {
        name: "",
        namespace: "",
  ...
  methods: {
    labelshow() {
      ...
      a.forEach((item) => {
        ...
        return JSON.stringify(Object.fromEntries(label));
        console.log("标签组为：", JSON.stringify(Object.fromEntries(label)));
      });
    },
    handleClose1() {
    ...
    getSvcs(name) {
      ...
      this.getSvcData.params.namespace = this.createWorkflow.namespace;
    ...
    showResourceInfo() {
      ...
      this.createWorkflowData.params.memory =
        this.createWorkflow.resource.split("/")[1];
    ...
    handleClose(done) {
      this.$confirm("还有未保存的工作哦确定关闭吗？")
        .then((_) => {
          ...
          this.createWorkflowData.params.type = "";
          console.log("重置后表单: ", this.createWorkflow);
        })
        .catch((_) => {});
    },
    //获取namespace列表
    ...
    //提交workflow参数
    // submitForm(formName) {
    //   //验证表单的每个规则是否通过，通过则调用createWorkflowFunc，反之返回false
    //   this.$refs[formName].validate((valid) => {
    //     if (valid) {
    //       this.createWorkflowFunc();
    //     } else {
    //       return false;
    //     }
    //   });
    ...
    deleteWorkflow(obj) {
      ...
      this.deleteWorkflowData.params.name = obj;
      this.deleteWorkflowData.params.namespace = this.namespaceValue;
      httpClient
    ...
    //获取workflow详情
    getWorkflowDetail(obj) {
      this.workflowDetailData.params.name = obj.metadata.name;
      if (this.namespaceValue != "") {
        this.workflowDetailData.params.namespace = this.namespaceValue;
      }
      this.workflowDetailData.params.namespace = obj.metadata.namespace;
      httpClient
        .get(this.workflowDetailData.url, {
          params: this.workflowDetailData.params,
        })
        .then((res) => {
          console.log("workflow详情为：", res.data);
